...
class Messages extends Component {
  ...
  link(str) {
    var reg = /(http:\/\/|https:\/\/)((\w|=|\?|\.|\/|&|-)+)/gi;
    return str.replace(
      reg,
      '<a className="link" target="_bank" href="$1$2">$1$2</a>'
    );
  }
  render() {
            ...
            <ul>
              ...
              {_currentChat.messages.map((item, i) => {
                return (
                  <li key={i}>
                    {i != 0 &&
                    this.time(item.date, _currentChat.messages[i - 1].date) !=
                      "" ? (
                      <p className="time">
                        <span>
                          {this.time(
                            item.date,
                            _currentChat.messages[i - 1].date
                          )}
                        </span>
                      </p>
                    ) : null}
                    ...
                    <div className={classnames("main", { self: item.self })}>
                      <img
                        ...
                        width="35"
                        height="35"
                        src={item.self ? _user.img : _currentChat.user.img}
                      />
                      <div
                        className="text"
                        dangerouslySetInnerHTML={{
                          __html: this.link(item.content)
                        }}
                      />
                    </div>
                  </li>
        ...
        <div className="dialog">
          ...
          <div className="dia-cont">
            <div className="clearfix">
              <p className="avatar">
                <img
                  src="https://ps.ssl.qhimg.com/t01531c2d8bd3dbe644.jpg"
                  alt=""
                />
              </p>
